...
$this->title = 'Edit Popular Restaurants';
			  ...
			  <div class="form-group">
				...
				<select class="js-example-basic-multiple form-control" name="popularRestaurants[]" multiple="multiple">
				   <?php foreach($dataFromRestaurantDisplay as $res) { ?>
					<option <?php if(in_array($res['restaurant_id'], $popularResraturantsResult)) { ?> selected = 'selected' <?php } ?> value='<?= $res['restaurant_id']; ?>'><?= $res['restaurant_name']."ID:".$res['restaurant_id']; ?></option>
					<?php } ?>
				</select>
			  </div>
			  </div>
